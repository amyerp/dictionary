...
func GetCurrencies(t *pb.Request) (response *pb.Response) {
	...
	var count int64
	db.Conn.Debug().Model(data).Group("currency").Count(&count)
	db.Conn.Debug().Group("currency").Limit(limit).Offset(offset).Find(&data)

	ans["currencies"] = data
	ans["currenciescount"] = count
